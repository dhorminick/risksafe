...
/** Include PHPExcel */
$as = new assessment();
if (isset($_REQUEST["action"]) && $_REQUEST["action"]=="downloadxlsforaml") {
	...
	$fileName = "Anti Money Laundering	 Summary Report" . date('Y-m-d') . ".xls";
  
	// Column names
	$fields = array('Type of Assessment', 'Team or Company', 'Task/Process', 'Business/Process Owner', 'Assessment #', 'Approval');
	...
	$excelData .= '<tr>';
	$lineData = array($assessment['ty_name'], $assessment['as_team'], $assessment['as_task'], $assessment['as_owner'], $assessment['idassessment'], $as->getApproval($assessment['as_approval']));
