...
if ($_REQUEST["action"] == "editdetail") {
  $edit = true;
  $id = $_REQUEST["id"];
  $datainfo = $assess->getAssessmentDet($_REQUEST["id"]);
} else {
  $id = -1;
  $edit = false;
}
...
<body>
  ...
  <div class="container-fluid">
    <div class="row">
      ...
      <div class="col-lg-9 col-md-12">
        <h1 class="page-header">Risk Details</h1>
        <div class="col-lg-9 col-md-12">
          <div class="panel panel-default">
            <div class="panel-body">
              <form role="form" id="form" action="../controller/assessment.php">
                <div class="alert alert-info" id="notify" <?php if (!isset($msg)) { ?>style="display: none;" <?php } ?>>
                  <?php if (isset($msg)) echo $msg; ?>
                </div>
                <div class="form-group">
                  <button type="button" class="btn btn-md btn-info pull-right" id="btn_viewdet">Switch to assessment Details</button>
                </div>
                ...
                <div class="form-group">
                  ...
                  <input name="date" id="date" type="text" maxlength="100" class="form-control readonly" placeholder="Select date..." required readonly style="cursor:pointer;" value="<?php if ($edit) {
                                                                                                                                                                                          echo date("m/d/Y", strtotime($datainfo["as_duedate"]));
                                                                                                                                                                                        } else {
                                                                                                                                                                                          echo date("m/d/Y");
                                                                                                                                                                                        } ?>">
                </div>
  ...
  <script>
    $(document).ready(function(e) {
      ...
      $("#risk_div").load('../controller/assessment.php?action=listrisks&type=' + riskType + '&selected=' + selrisk);
      $("#hazard_div").load('../controller/assessment.php?action=listhazards&cat=' + cathazard + '&selected=' + selhazard);
      ...
      $("#rating").load('../controller/assessment.php?action=rating&likelihood=' + $("#likelihood").val() + '&consequence=' + $("#consequence").val());

      $("#risk_div").change(function(e) {

        $("#hazard_div").load('../controller/assessment.php?action=listhazards&cat=' + $("#risk").val() + '&selected=' + selhazard);
      ...
      $("#existing_ct").change(function(e) {
        if ($(this).val() !== '-1') {
          ...
          $.ajax({
            type: "GET",
            url: "../controller/assessment.php?action=addlibcontrol&id=" + $(this).val() + '&det=' + detid,
            async: false
          })
          // $("#controls").load('../controller/assessment.php?action=listcontrols&id=' + detid);
          // $("#control").val("");
      ...
      $("#existing_tr").change(function(e) {
        if ($("#exisitng_tr").val() !== '-1') {
          $.ajax({
            type: "GET",
            url: "../controller/assessment.php?action=addlibtreat&id=" + $("#existing_tr").val() + '&det=' + detid,
            async: false
          })
          $("#treatments").load('../controller/assessment.php?action=listtreat&id=' + detid);
          $("#treatment").val("");
