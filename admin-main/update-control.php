<?php
    ...
    if(isset($_POST['importSubmit']) && isset($_POST["ri_id"])){
        ...
        if($error == false){
            ...
            $InsertRisk = "UPDATE as_newrisk SET control = '$n' WHERE risk_id = '$risk_id'";
...
<head>
  ...
  <title>Update Controls | <?php echo $siteEndTitle; ?></title>
                ...
                <div class="section-body">
                    <div class="card" style="padding: 10px;">
                        ...
                        <div class="card-header">
                            <h4 class="d-inline">Update Controls:</h4>
                        ...
                        <div class='card-body'>
                            <form action="" method="post">
                                ...
                                <div class='custom'>
                                <?php
                                    $CheckIfAssessmentsExist = "SELECT * FROM as_newrisk WHERE risk_id = '$id'";
                                    $AssessmentsExist = $con->query($CheckIfAssessmentsExist);
                                    if ($AssessmentsExist->num_rows > 0) {
                                        $hasSubs = true;
                                     $data = $AssessmentsExist->fetch_assoc();
                                     $subrisks = unserialize($data['control']);
                                     $i=0; $count = 0;
                                     foreach($subrisks as $value){ $i++; $count++;
                                ?>
                                ...
                                <?php }}else{ ?>
                                <div class='' style='width:100%;padding:20px 10px;'>
                                <h4>No Control Added Yet!!</h4>
                                </div>
                                <? } ?>
                                </div>
                                ...
                                <div class="form-group text-right" style='margin-top:30px !important;'>
                                    <button type="submit" class="btn btn-primary btn-lg btn-icon icon-left" name="importSubmit"><i class="fa fa-plus"></i> Update Controlk</button>
                                </div>
                                <?php } ?>
                            </form>
                        </div>
